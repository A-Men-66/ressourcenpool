...
app.get('/api/ressourcen/public', async (req, res) => {
    try {
        const { rows } = await pool.query(`
            SELECT r.id, r.name, r.beschreibung, r.typ, r.plz, r.status,
                COALESCE(json_agg(DISTINCT rt.tag_name) FILTER (WHERE rt.tag_name IS NOT NULL), '[]') AS tags,
                (SELECT bild_pfad FROM ressource_bilder WHERE ressource_id = r.id ORDER BY position LIMIT 1) AS bild
            FROM ressourcen r
            LEFT JOIN ressource_tags rt ON r.id = rt.ressource_id
            WHERE r.status IN ('verfuegbar', 'angefragt')
            GROUP BY r.id
            ORDER BY r.id DESC
            LIMIT 12
        `);
        ...
        console.error('public ressourcen:', err);
        res.status(500).json({ error: 'Fehler' });
    }
});
...
app.get('/api/config', async (req, res) => {
    try {
        const { rows } = await pool.query(
            'SELECT app_name, app_untertitel, gemeinschaftskompass, landing_modus FROM pool_profil WHERE id = 1'
        );
        const r = rows[0] || {};
        res.json({
            ...
            landing_modus:        r.landing_modus        || 'A'
        });
    } catch (err) {
...
app.get('/api/pool-profil', authenticate, requireAdmin, async (req, res) => {
    try {
        const { rows } = await pool.query(
            'SELECT name, admin_kontakt, plz, app_name, app_untertitel, gemeinschaftskompass, landing_modus FROM pool_profil WHERE id = 1'
        );
        res.json(rows[0] || { name: '', admin_kontakt: '', plz: '', app_name: '', app_untertitel: '', gemeinschaftskompass: '', landing_modus: 'A' });
    } catch (err) {
...
app.put('/api/pool-profil', authenticate, requireAdmin, async (req, res) => {
    const { name, admin_kontakt, plz, app_name, app_untertitel, gemeinschaftskompass, landing_modus } = req.body;
    try {
        await pool.query(
            `INSERT INTO pool_profil (id, name, admin_kontakt, plz, app_name, app_untertitel, gemeinschaftskompass, landing_modus)
             VALUES (1, $1, $2, $3, $4, $5, $6, $7)
             ON CONFLICT (id) DO UPDATE SET name=$1, admin_kontakt=$2, plz=$3, app_name=$4, app_untertitel=$5, gemeinschaftskompass=$6, landing_modus=$7`,
            [name || '', admin_kontakt || '', plz || '', app_name || '', app_untertitel || '', gemeinschaftskompass || '', landing_modus || 'A']
        );
        broadcast('update');
        res.json({ ok: true });
    } catch (err) {
...
async function initDB() {
    ...
    try {
        ...
        await client.query(`ALTER TABLE pool_profil ADD COLUMN IF NOT EXISTS landing_modus VARCHAR(1) DEFAULT 'A'`);
...
app.post('/webhook/deploy', (req, res) => {
  ...
  exec('cd /var/www/ressourcenpool && git pull origin main && npm install --production && pm2 restart ressourcenpool',
    (err, stdout, stderr) => {
